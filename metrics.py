...
class MetricsTracker:
    def __init__(self):
        ...
        self.stable_steps = 0
        self.false_alarms = 0 # Количество событий ложных срабатываний
    ...
    def calculate_hdd(self):
        # Ad (Recall/Sensitivity) - доля пойманных дрейфов
        total_inj = len(self.injected_drifts)
        ad = len(self.detected_drift_ids) / total_inj if total_inj > 0 else 1.0
        
        # A0 (Reliability) - доверие к сигналу (событийный подход)
        # Насколько часто сигнал детектора оказывается истинным
